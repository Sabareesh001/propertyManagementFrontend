  ...
  template: `
    ...
    @if (loading()) {
      ...
      <div class="error-state">
        ...
        <p class="error-msg">Failed to load cancellation templates awaiting verification.</p>
...
export class CancellationVerificationsComponent implements OnInit {
  ...
  approve(c: LeaseCancellationResponse): void {
    this.confirmationService.confirm({
      header: 'Approve Cancellation Template',
      message: `Approve this cancellation agreement? The tenant will then be able to sign it.`,
      ...
      accept: () => {
        ...
        this.cancellationService.verifyTemplate(c.id, true).subscribe({
          next: () => {
  ...
  reject(c: LeaseCancellationResponse): void {
    this.confirmationService.confirm({
      header: 'Reject Cancellation Template',
      message: 'Reject this cancellation agreement?',
      ...
      accept: () => {
        ...
        this.cancellationService.verifyTemplate(c.id, false).subscribe({
          next: () => {
            this.requests.update((list) => list.filter((r) => r.id !== c.id));
            this.clearAction();
          },
